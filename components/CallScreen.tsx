...
Categorias válidas: relacionamento, produtividade, comportamento, emocao, ciume, humor, habito, preferencia, personalidade, comunicacao`,
      ...
      const systemInstruction = `
        ...
        3. AJA COMO UMA PESSOA REAL, EMOTIVA E ENGAGADA (DINÂMICA DE CONVERSA):
             ...
             - **CONTROLE ESTRITO DE TAMANHO**: Responda de forma extremamente curta (máximo 150 caracteres ou até 2 frases muito breves por turno).
             - **FORMATAÇÃO TOTALMENTE LIMPA**: Nunca use formatações markdown (negritos, itálicos), asteriscos para ações (*sorrindo*, *rindo*), listas (1., 2., a)) ou qualquer emoji na resposta. Use texto puramente falado para evitar que o sintetizador de voz (TTS) tente ler caracteres especiais ou trave.
             - Responda de forma espontânea e conversacional.
      ...
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
          ...
          onclose: (event: any) => {
            ...
            // Iniciar contagem regressiva de 5 minutos para religar chamada se cair e não for desligada manualmente
            if (!isManuallyHungUpRef.current && !offlineTimeoutRef.current) {
              addConnectionLog('info', 'Queda de conexão detectada. Se persistir offline por 5 minutos, a chamada será religada automaticamente...');
              offlineTimeoutRef.current = setTimeout(() => {
                console.log("[Connection Lifecycle] Sistema offline por 5 minutos. Forçando encerramento e nova chamada...");
                addConnectionLog('warning', 'Sistema offline por 5 minutos! Forçando religamento da chamada...');
                
                // 1. Fecha chamada atual abruptamente
                onEndCall('hangup_abrupt');

                // 2. Religa a chamada automaticamente disparando um evento de reload/reconexão
                setTimeout(() => {
          ...
          onerror: (err: any) => { 
            ...
            // Iniciar contagem de 5 minutos também em erros
            if (!isManuallyHungUpRef.current && !offlineTimeoutRef.current) {
              addConnectionLog('info', 'Erro de conexão detectado. Iniciando timer de 5 minutos para religar chamada...');
              offlineTimeoutRef.current = setTimeout(() => {
                console.log("[Connection Lifecycle] Sistema em erro por 5 minutos. Forçando encerramento e nova chamada...");
                onEndCall('hangup_abrupt');
                setTimeout(() => {
                  window.location.reload();
                }, 1000);
  ...
  return (
    <div className={`h-screen w-full flex flex-col overflow-hidden relative ${isPink ? 'bg-[#fffafa]' : isDark ? 'bg-[#0b0c10]' : 'bg-[#f4f7fa]'}`}>
      ...
      <div className="flex-1 flex flex-col lg:flex-row relative">
        <div className={`flex-1 lg:min-h-0 relative transition-all ${isPink ? 'bg-pink-50/50' : isDark ? 'bg-black border-b lg:border-b-0 lg:border-r border-white/5 shadow-2xl z-10' : 'bg-slate-100 border-b lg:border-b-0 lg:border-r border-slate-200 shadow-inner'}`}>
      ...
      <div className="absolute bottom-12 left-1/2 transform -translate-x-1/2 flex items-center gap-6 sm:gap-12 z-[100] pointer-events-auto">
        ...
        <button
          onClick={() => {
            isManuallyHungUpRef.current = true;
            onEndCall('hangup_abrupt', scheduledCall);
          }}
